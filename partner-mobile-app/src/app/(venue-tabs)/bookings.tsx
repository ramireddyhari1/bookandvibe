...
export default function BookingsScreen() {
  ...
  async function fetchBookings() {
    try {
      const response = await apiClient.get('/partners/venue-bookings', {
        params: { status: filter === 'UPCOMING' ? 'CONFIRMED' : filter }
      });
  ...
  const renderBookingItem = ({ item }: any) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        <View style={[styles.tag, { backgroundColor: item.status === 'USED' ? COLORS.primary + '20' : COLORS.success + '20' }]}>
          <Text style={[styles.tagText, { color: item.status === 'USED' ? COLORS.primary : COLORS.success }]}>
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>{new Date(item.bookingDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })} • {item.slotLabel || 'Standard'}</Text>
        </View>
        <View style={styles.infoRow}>
      ...
      <View style={styles.cardFooter}>
        <Text style={styles.amount}>₹{item.totalAmount || item.facility?.pricePerHour || 0}</Text>
        {item.status !== 'USED' ? (
          <TouchableOpacity 
            style={styles.actionButton}
            onPress={() => router.push('/(venue-tabs)/scan')}
          >
            <CheckCircle2 size={16} color={COLORS.primary} />
            <Text style={styles.actionButtonText}>VERIFY QR</Text>
          </TouchableOpacity>
        ) : (
          <View style={styles.actionButton}>
            <CheckCircle2 size={16} color={COLORS.textMuted} />
            <Text style={[styles.actionButtonText, { color: COLORS.textMuted }]}>VERIFIED</Text>
          </View>
        )}
      </View>
    </View>
