...
import { mockFacilities, mockSlots } from '../../src/data/partner';
...
export default function SlotsScreen() {
  ...
  const selectedTemplate = selectedFacility?.slotTemplate?.length
    ...
    : mockSlots.map((slot) => ({ label: slot.time, price: slot.basePrice, weekendPrice: slot.suggestedPrice }));
  ...
  async function fetchFacilities() {
    try {
      ...
      const items = Array.isArray(response.data?.data) && response.data.data.length ? response.data.data : mockFacilities;
      setFacilities(items);
      setSelectedFacilityId((current) => current || items[0]?.id || '');
    } catch (error) {
      console.error('Failed to fetch facilities', error);
      setFacilities(mockFacilities as Facility[]);
      setSelectedFacilityId((current) => current || mockFacilities[0]?.id || '');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function fetchFacilityState(facilityId: string, dateText: string) {
    try {
      ...
      const fallback = facilities.find((item) => item.id === facilityId) || mockFacilities[0];
      setSelectedFacility(fallback as Facility);
      setAvailability(mockSlots.map((slot) => ({ label: slot.time, status: slot.status === 'AVAILABLE' ? 'AVAILABLE' : slot.status === 'BOOKED' ? 'BOOKED' : 'BLOCKED' })));
      setCalendarDays([]);
    }
  }
